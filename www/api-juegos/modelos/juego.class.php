...
class Juego extends Database
{
    ...
    private function validate($data)
    {
        ...
        /*
        Tengo que comprobar la extensión de la imagen del juego
        */
	...
	public function insert($params)
	{
		...
		if($this->validate($params)){
			...
			if (isset($params['imagen'])){
				...
				//del directorio actual de user.class, subo un nivel (1) y estando en el directorio api-juegos, concateno public\img
		...
		if($this->validate($params)){
			...
			*/
			if (isset($params['imagen'])){
				...
				$juegos = parent::getDB($this->table, $_GET);
				$juego = $juegos[0];
				$imagen_antigua = $juego['imagen'];
				...
				if ($imagen_antigua != null && !unlink($path)){
						$response = array(
							'result' => 'warning',
							'details' => 'No se ha podido eliminar la imagen actual del juego'
						);	
						Response::result(200, $response);
						exit;
 ...
 * También hay que eliminar la imagen de ese juego.
 */
	public function delete($id)
	{
		...
		if(!empty($imagen_antigua)){
			...
			if (!unlink($path)){
				$response = array(
					'result' => 'warning',
					'details' => 'No se ha podido eliminar la imagen del juego'
				);	
				Response::result(200, $response);
				exit;
